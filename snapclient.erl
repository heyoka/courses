...
-export([start_link/1, stop/1,
   ...
   connect/1, disconnect/1, get_params/2, set_params/3]).
...
-export([set_session_password/2, clear_session_password/1, get_protection/1]).
...
%%
-spec read_multi_vars(pid(), list()) -> {ok, binary()} | {error, einval} | {error,term()}.
...
init([]) ->
   ...
   lager:info("Port: ~p, from ~p",[Port, Executable]),
   {ok, #state{port = Port}}.
...
handle_call(get_cpu_info, _From, State) ->
   {reply, call_simple(State, get_cpu_info), State};
...
call_port(State = #state{port = Port}, Command, Args, Timeout) ->
   Msg = {Command, Args},
